...
class Red5GrailsPlugin {
    ...
    def description = '''\
Makes it relatively easy to use the Red5 functionality within Grails.  The goals of this plugin are as follows:

    * Provide an easy mechanism to develop new Red5 applications in Grails
    * Allow Red5 applications to use GORM
...
'''
   ...
   private configureRed5AppBeans = { red5AppClass ->
    ...
    def onChange = { event ->
        ...
        if(event.source) {
            ...
            if(event.ctx) {
                event.ctx.registerBeanDefinition(red5AppName, beans.getBeanDefinition(red5AppName))
            }
        }
        */
    }
